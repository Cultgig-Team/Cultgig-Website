...
export async function sendOtp(email: string) {
  if (!isAppwriteConfigured) {
    if (import.meta.env.DEV) return { userId: "local-preview", local: true };
    throw new Error(
      "Email verification is temporarily unavailable. Please try again later.",
    );
  }
  const token = await account.createEmailToken({ userId: ID.unique(), email });
  return { userId: token.userId, local: false };
}
export async function verifyOtp(
  ...
  if (local) {
    ...
    if (secret !== "123456")
      throw new Error("That code is invalid. Please check it and try again.");
    return;
  }
